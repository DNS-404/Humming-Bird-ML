...
public class Flower : MonoBehaviour
{
    ...
    /// <returns>The actual of amount successfully removed</returns>
    public float Feed(float amount)
    {
        ...
        if (NectarAmount <= 0)
        {
            ...
            // Disable the flower and nectar colliders (To prevent bird from getting stuck between two flowers)
            flowerCollider.gameObject.SetActive(false);
            nectarCollider.gameObject.SetActive(false);

            // Change flower color to indicate that it is empty
