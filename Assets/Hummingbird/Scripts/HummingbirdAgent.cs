...
public class HummingbirdAgent : Agent
{
    ...
    [Tooltip("Speed to roatate around the up axis")]
    ...
    public override void Initialize()
    {
        ...
        flowerArea = GetComponentInParent<FlowerArea>(); // parent of bird is the whole island

        // If not training mode, no max step, play forever
        if (!trainingMode) MaxStep = 0; // 0 here means infinite
    }
    ...
    /// </summary>
    /// <param name="vectorAction">A list of decisions by user or AI. Basically, the actions to take</param>
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // (+1 -> beak tip is directly in front of the flower)
        // (-1 -> beak tip is directly behind the flower)
        // Taking dot product of vector to the flower and vector into the flower.
        sensor.AddObservation(Vector3.Dot(toFlower.normalized, -nearestFlower.FlowerUpVector.normalized));

        // Observe a dot product that indicates whether the beak is pointing toward the flower (1 observation)
        // (+1 -> beak tip is pointing directly at the flower)
        // (-1 -> beak tip is pointing directly away from the flower)
        // Taking dot product of vector straight from beak tip and vector into the flower.
        sensor.AddObservation(Vector3.Dot(beakTip.forward.normalized, -nearestFlower.FlowerUpVector.normalized));

        // Observe relative distance from the beak tip to the flower (1 observation)
    ...
    {
        // Heuristic is basically some sort of algorithmic decision being made instead of a NN making the decision.
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        // Loop untill a safe position is found or we run out of attempts
        while(!safePositionFound && attemptsRemaining > 0)
        {
            ...
            else
            {
                ...
                // Combine the height, radius and direction pick a potential position
    ...
    /// <summary>
    /// Update the flower nearest to the agent (will not be called every update)
    /// </summary>
    ...
    /// <summary>
    /// Called when the agent's collider stays a trigger collider (called every update)
    /// </summary>
    /// <param name="other">The trigger collider</param>
    private void OnTriggerStay(Collider other)
    {
        // In case OnTriggerEnter doesn't work, we're adding OnTriggerStay as well.
        TriggerEnterOrStay(other);
    }
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // Note: this is per fixed timestamp, meaning it happens every .02 seconds, or 50x every second.
                ...
                if (trainingMode)
                {
                    // Calculate bonus for getting nectar (More reward if the beak is straight into the flower.. less if at some angle)
                    float bonus = .02f * Mathf.Clamp01(Vector3.Dot(transform.forward.normalized, -nearestFlower.FlowerUpVector.normalized));
                    
                    AddReward(.01f + bonus);
                }

                // If flower is mepty, update nearest flower
                if (!flower.HasNectar)
                {
                    UpdateNearestFlower();
                }
